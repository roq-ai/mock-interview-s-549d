...
export function ScheduleListPage(props: ScheduleListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getSchedules({
        relations: ['user_schedule_faculty_idTouser', 'user_schedule_student_idTouser'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_schedule_faculty_idTouser',
          header: 'User Schedule Faculty Id Touser',
          accessorKey: 'user_schedule_faculty_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_schedule_faculty_idTouser?.id}`}
            >
              {record.user_schedule_faculty_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_schedule_student_idTouser',
          header: 'User Schedule Student Id Touser',
          accessorKey: 'user_schedule_student_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_schedule_student_idTouser?.id}`}
            >
              {record.user_schedule_student_idTouser?.email}
            </Link>
          ),
        }
      : null,
